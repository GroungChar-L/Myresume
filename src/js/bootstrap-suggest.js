    ...
    var methods = {
        init: function(opts){
            ...
            return self.each(function(){
                var $input = $(this),
                    $dropdownMenu = $input.parents(".input-dropd:eq(0)").find("ul.dropdown-menu");
                //验证输入框对象是否符合条件
                ...
                if (! options.showBtn) {
                    $input.css('border-radius', '4px')
                        .parents(".input-dropd:eq(0)").css('width', '100%')
                        .find('.input-dropd-btn>.btn').hide();
                }
                ...
                $dropdownMenu.on("mouseenter", function(){
                    ...
                    //$input.blur();
                    //$(this).show();
                    //$input.parents(".input-dropd:eq(0)").find('.input-dropd-btn>.btn').click();
                ...
                if ($dropdownMenu.is(':visible')) {
                    setTimeout(function(){
                        if ( //自动判断菜单向上展开
                            ...
                            $dropdownMenu.parents('.input-dropd').addClass('dropup');
                        } else {
                            $dropdownMenu.parents('.input-dropd.dropup').removeClass('dropup');
                        }
                    }, 100);
                    //return;
                }
            ...
            */
            function checkInput(target) {
                var $input = $(target),
                    $dropdownMenu = $input.parent(".input-dropd").find("ul.dropdown-menu"),
...
//                    console.log("-=-=-=kaishi")
//                    console.log(data.value[i].__index);
//                    console.log(idValue);
...
//                    console.log("--=-=-=jieshu");
        ...
        disable: function (e) {
            ...
            $(this).attr("disabled", true).parent().find(".input-dropd-btn>.btn").addClass("disabled");
        },
        enable: function () {
            if(!$(this).data("bsSuggest")) {
                return false;
            }
            $(this).attr("disabled", false).parent().find(".input-dropd-btn>.btn").removeClass("disabled");
        },
        destroy: function(){
            $(this).off().removeData("bsSuggest").parent().find(".input-dropd-btn>.btn").off();//.addClass("disabled");
        },
        version: function() {
            return '0.0.1';
        }
